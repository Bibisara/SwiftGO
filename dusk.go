...
func (s *SwiftGO) TakeScreenShot(pageURL, testName string, w, h float64) {
	...
	_ = utils.OutputFile(fmt.Sprintf("%s/screenshots/%s-%s.png", s.RootPath, testName, fileName), img)
}

func (s *SwiftGO) FetchPage(pageURL string) *rod.Page {
	return rod.New().MustConnect().MustIgnoreCertErrors(true).MustPage(pageURL).MustWaitLoad()
}

func (s *SwiftGO) SelectElementByID(page *rod.Page, id string) *rod.Element {
